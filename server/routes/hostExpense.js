...
router.get('/by-record/:recordId', requireAuth, async (req, res) => {
    try {
        ...
        const rows = await query(
            'SELECT id, unit_price, expense_date, created_at FROM host_expense_details WHERE record_id = ? ORDER BY expense_date ASC, id ASC',
            [recordId]
        );
...
router.post('/', requireAuth, async (req, res) => {
    try {
        const userId = req.session.userId;
        const { record_id, unit_price, expense_date } = req.body;
        ...
        const result = await execute(
            'INSERT INTO host_expense_details (user_id, record_id, unit_price, expense_date) VALUES (?, ?, ?, ?)',
            [userId, record_id, price, date]
        );
        logAudit(userId, 'host_expense_add', `record=${record_id} price=${price} date=${date}`);
        res.json({ success: true, id: result.lastID, expense_date: date, unit_price: price });
    } catch (err) {
...
router.put('/:id', requireAuth, async (req, res) => {
    try {
        ...
        const { unit_price, expense_date } = req.body;
