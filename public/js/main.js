...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function switchTab(tabId, force = false) {
        ...
        if (state.currentTabId) {
            ...
            if (tw) state.tabScrollTops[state.currentTabId] = tw.scrollTop;
        ...
        renderTable(false);
        // 恢复目标标签的滚动位置
        const savedScrollTop = state.tabScrollTops[tabId] || 0;
        if (savedScrollTop > 0) {
    ...
    function bindTableEvents() {
        ...
        if (!window._dragSelectBound) {
            ...
            // 根据行 ID 获取在当前表格中的 DOM 索引位置
            function getRowDomIndex(rowId) {
                var rows = getVisibleRows();
                for (var i = 0; i < rows.length; i++) {
                    if (parseInt(rows[i].dataset.id) === rowId) return i;
                }
                return -1;
            }

            // 获取指定列在某行中的可编辑目标元素（用于添加/移除 drag-selected）
            function getCellTargetInRow(tr, colKey) {
                // 优先找特殊单元格（密码、日期）
                var specialCell = tr.querySelector('.password-cell[data-col="' + colKey + '"], .date-cell[data-col="' + colKey + '"]');
                if (specialCell) return specialCell;
                // 普通 input 单元格
                var input = tr.querySelector('.cell-input[data-col="' + colKey + '"]');
                return input;
            }
            ...
            function updateDragHighlight() {
                ...
                if (!ds.colKey) return;
                if (ds.startRowId === null || ds.endRowId === null) return;

                var startIdx = getRowDomIndex(ds.startRowId);
                var endIdx = getRowDomIndex(ds.endRowId);
                ...
                for (var i = minIdx; i <= maxIdx; i++) {
                    var target = getCellTargetInRow(rows[i], ds.colKey);
                    if (target) target.classList.add('drag-selected');
                }
            }

            // 按 DOM 显示顺序获取选中范围内的行 ID
            function getSelectedRowIds(ds) {
                if (ds.startRowId === null || ds.endRowId === null) return [];
                var startIdx = getRowDomIndex(ds.startRowId);
                var endIdx = getRowDomIndex(ds.endRowId);
                if (startIdx === -1 || endIdx === -1) return [];
                ...
                var ids = [];
                for (var i = minIdx; i <= maxIdx; i++) {
                    var rid = parseInt(rows[i].dataset.id);
            ...
            document.addEventListener('copy', function(e) {
                ...
                const targetRowIds = getSelectedRowIds(ds);
    ...
    function bindSpecialEvents() {
        ...
        $$('.months-input').forEach(input => {
            input.onchange = async function() {
                const id = parseInt(this.dataset.id);
                const record = state.records.find(r => r.id === id);
                if (!record) { handleCellChange(this); return; }
                // 独享/共享标签：检测月数变化，同步明细（增删差额笔数）
                if (!isSimpleTab()) {
                    ...
                    if (diff > 0) {
                        ...
                        // 删除 |diff| 笔最晚的明细
                        for (let i = 0; i < -diff; i++) {
                            if (!record._hostExpenseDetails || record._hostExpenseDetails.length === 0) break;
                            try {
                                await API.delete('/host-expense/latest/' + id);
                                record._hostExpenseDetails.sort((a, b) => {
                                    const da = a.expense_date || '', db = b.expense_date || '';
                                ...
                                record._hostExpenseDetails.pop();
                            } catch (err) { console.error('同步删除明细失败:', err); }
                        }
                        record._hostExpenseTotal = (record._hostExpenseDetails || []).reduce((s, d) => s + (parseFloat(d.unit_price) || 0), 0);
                    }
                }
        ...
        // 支出交互（独享/共享标签：点击直接打开主机支出明细弹窗；普通记账标签：旧逻辑切换编辑）
        $$('.expense-display').forEach(display => {
            display.addEventListener('click', function(e) {
                ...
                const record = state.records.find(r => r.id === id);
                if (!record) return;
                // 独享/共享标签：无论有无明细，点击都打开弹窗
                if (!isSimpleTab()) {
                    openHostExpenseModal(id);
                    return;
                }
                // 普通记账标签：保持旧的"点击切换为编辑"逻辑（简单标签的支出列走 _expenseTotal 弹窗，不会走到这里）
    ...
    function doMultiPaste(ds, clipboardText) {
        ...
        for (var i = 0; i < rows.length; i++) {
            var rid = parseInt(rows[i].dataset.id);
            if (rid === ds.startRowId) startIdx = i;
            if (rid === ds.endRowId) endIdx = i;
        }
        if (startIdx === -1 || endIdx === -1) return;
        var minIdx = Math.min(startIdx, endIdx);
        var maxIdx = Math.max(startIdx, endIdx);
        ...
        for (var j = minIdx; j <= maxIdx; j++) {
            var rid2 = parseInt(rows[j].dataset.id);
            if (!isNaN(rid2)) targetRowIds.push(rid2);
        }
        if (targetRowIds.length === 0) return;
    ...
    async function renderColumnList() {
        ...
        cols.forEach(col => {
            ...
            html += `
                <div class="column-item" draggable="true" data-id="${col.id}">
                    <div class="col-info">
                        <span class="col-name-wrapper">
                            <strong class="col-name-display" data-id="${col.id}" style="cursor:pointer;" title="双击编辑列名">${escapeHtml(col.col_name)}</strong>
                            <input type="text" class="col-name-input" data-id="${col.id}" style="display:none;min-width:120px;padding:2px 4px;border:1px solid #4a6cf7;border-radius:4px;font-size:13px;" />
                        </span>
        ...
        // 双击列名 → 编辑列名
        columnList.querySelectorAll('.col-name-display').forEach(display => {
            display.addEventListener('dblclick', function(e) {
                e.stopPropagation();
                const id = parseInt(this.dataset.id);
                const col = state.columns.find(c => c.id === id);
                if (!col) return;
                const wrapper = this.closest('.col-name-wrapper');
                if (!wrapper) return;
                const input = wrapper.querySelector('.col-name-input');
                if (!input) return;
                input.value = col.col_name;
                this.style.display = 'none';
                input.style.display = 'inline-block';
                input.focus();
                input.select();
        ...
        columnList.querySelectorAll('.col-name-input').forEach(input => {
            const saveEdit = async () => {
                const id = parseInt(input.dataset.id);
                const col = state.columns.find(c => c.id === id);
                if (!col) return;
                const wrapper = input.closest('.col-name-wrapper');
                const display = wrapper ? wrapper.querySelector('.col-name-display') : null;
                const newName = input.value.trim();
                if (!newName || newName === col.col_name) {
                    if (display) display.style.display = '';
                    input.style.display = 'none';
                    return;
                }
                try {
                    await API.put('/columns/' + id, { col_name: newName });
                    col.col_name = newName;
                    updateTabCache(state.currentTabId);
                    renderColumnList();
                    renderTable(false);
                    setStatus('✅ 列名已更新');
                    ...
                    setStatus('❌ 更新失败: ' + err.message);
                    if (display) {
                        display.textContent = escapeHtml(col.col_name);
    ...
    document.addEventListener('contextmenu', function(e) {
        ...
        if (isDedicated) {
            // ===== 独享标签：3个提取 + 3个粘贴（互斥）=====
            // 隐藏共享专用菜单
            ctxAddClient.style.display = 'none';
            ctxCopyClient.style.display = 'none';
            ctxPasteClient.style.display = 'none';

            // 粘贴菜单：根据提取类型互斥显示，且需满足条件
            // 粘贴服务器+客户信息：收入列和支出列都是0
            ctxPasteBoth.style.display = state.extractedBothData && incomeTotal === 0 && expenseTotal === 0 ? 'block' : 'none';
            // 粘贴服务器信息：支出列是0
            ctxPasteServer.style.display = state.copiedServerData && expenseTotal === 0 ? 'block' : 'none';
            // 粘贴客户信息：收入列是0
            ctxPasteExtractedClient.style.display = state.extractedClientData && incomeTotal === 0 ? 'block' : 'none';

            const anyPasteVisible = ctxPasteBoth.style.display === 'block'
                || ctxPasteServer.style.display === 'block'
                || ctxPasteExtractedClient.style.display === 'block';

            if (anyPasteVisible) {
                // 有可粘贴项时：只显示对应粘贴，隐藏所有提取（用户要求"只有粘贴"）
                ctxExtractBoth.style.display = 'none';
                ctxCopyServer.style.display = 'none';
                ctxExtractClient.style.display = 'none';
            } else {
                // 无可粘贴项时：显示3个提取菜单（允许提取/重新提取）
                ctxExtractBoth.style.display = 'block';
                ctxCopyServer.style.display = 'block';
                ctxExtractClient.style.display = 'block';
            }
        } else {
            // ===== 共享/简单标签：保持原有逻辑 =====
            // 隐藏独享专用菜单
            ...
            // 服务器提取/粘贴（共享+simple，原有逻辑：空白行条件）
            ctxCopyServer.style.display = (isShared || isSimple) && isServerRow ? 'block' : 'none';
            ctxPasteServer.style.display = (isShared || isSimple) && isServerRow && state.copiedServerData && isEmptyRow ? 'block' : 'none';
        }
    ...
    // ===== 独享标签：提取/粘贴 服务器信息 =====
    ctxCopyServer.addEventListener('click', async () => {
        contextMenu.style.display = 'none';
        if (!contextTargetId) return;
        const record = state.records.find(r => r.id === contextTargetId);
        if (!record) return;
    ...
    ctxExtractClient.addEventListener('click', () => {
        ...
        const record = state.records.find(r => r.id === contextTargetId);
        if (!record) return;
        // 客户信息：客户购买时间、客户到期时间、客户剩余天数、客户名、单价备注、收入明细
        const clientData = {};
        CLIENT_FIELDS.forEach(k => { clientData[k] = record.data[k] !== undefined ? record.data[k] : ''; });
        ...
        const tr = document.querySelector(`tr[data-id="${contextTargetId}"]`);
        if (tr) tr.classList.add('cut-pending');
        setStatus('客户信息已提取 ✂️，请在收入列为0的行右键粘贴');
    });

    ctxPasteExtractedClient.addEventListener('click', async () => {
        contextMenu.style.display = 'none';
        if (!contextTargetId || !state.extractedClientData || !state.currentTabId) {
            setStatus('无法粘贴：缺少目标或提取的数据');
            return;
        }
        const targetRec = state.records.find(r => r.id === contextTargetId);
        if (!targetRec) return;
        // 粘贴条件：收入列为0
        const incTotal = targetRec._incomeTotal || 0;
        if (incTotal !== 0) {
            setStatus('粘贴失败：目标行收入列不为0');
            return;
        }
        try {
            setStatus('粘贴客户信息中...');
    ...
    ctxExtractBoth.addEventListener('click', async () => {
        ...
        const tr = document.querySelector(`tr[data-id="${contextTargetId}"]`);
        if (tr) tr.classList.add('cut-pending');
        setStatus('服务器+客户信息已提取 ✂️，请在收入和支出列都为0的行右键粘贴');
    });
    ...
    ctxPasteBoth.addEventListener('click', async () => {
        ...
        // 粘贴条件：收入列和支出列都是0
        const incTotal = targetRec._incomeTotal || 0;
        const expTotal = targetRec._hostExpenseTotal || targetRec._expenseTotal || 0;
        if (incTotal !== 0 || expTotal !== 0) {
            setStatus('粘贴失败：目标行收入列或支出列不为0');
            return;
        }
        try {
            ...
            // 2. 将服务器+客户字段写入目标行
            const updateData = { ...targetRec.data };
            [...SERVER_FIELDS, ...CLIENT_FIELDS].forEach(k => {
                updateData[k] = src.data[k] !== undefined ? src.data[k] : '';
            });
            await API.put('/records/' + contextTargetId, { data: updateData });
    ...
    if (quoteCopyImgBtn) quoteCopyImgBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            if (imgErr || !img.complete || img.naturalWidth === 0) {
                // 兜底：Blob URL
                try {
                    const fbBlob = new Blob([svg], { type: 'image/svg+xml;charset=utf-8' });
